...
import me.leep.jee.commons.dao.ICrudDao;
...
//	}
	...
	public void testQuery() {
		...
		List<?> list = userCrudDao.query(sql);
		assertEquals(1, list.size());
		logger.info("用户名称" + ((UserBean)list.get(0)).getName());
		logger.info("全部记录条数: " + userCrudDao.countAll());
		Map<String, Object> paramsMap = new HashMap<String, Object>();
		paramsMap.put("number", "admin");
		List<UserBean> users = userCrudDao.findByEqual(paramsMap);
